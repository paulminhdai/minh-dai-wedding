...
async function getRollStatus(deviceId) {
    ...
    const { data, error } = await supabaseAdmin
        .from('photographers')
        .select('id, display_name, photo_count, is_blocked')
        .eq('device_id', deviceId)
        .maybeSingle();
...
/**
 * List photos for a specific device. Used for the "My Roll" photobooth feature.
 */
async function listMyPhotos(deviceId) {
    if (!deviceId) return [];
    ...
    const { data, error } = await supabaseAdmin
        .from('photos')
        .select('id, public_url, thumbnail_url, width, height, caption, created_at')
...
async function deletePhoto(photoId) {
    ...
    // Decrement the photographer's shot count so they get that slot back.
    // The DB trigger only fires on hard DELETE rows; soft-delete needs this.
    ...
    if (pg) {
        await supabaseAdmin
            .from('photographers')
            .update({ photo_count: Math.max((pg.photo_count || 1) - 1, 0) })
            .eq('id', photo.photographer_id);
    }
